...
router.post('/', (req, res) => {
  ...
  const info = db.prepare(`
    INSERT INTO tbl_to_do (summary_id, title, description, status, priority, due_date, position)
    VALUES (?, ?, ?, ?, ?, ?, ?)
...
router.delete('/:id', (req, res) => {
  db.prepare(
    "UPDATE tbl_to_do SET deleted_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = ?"
