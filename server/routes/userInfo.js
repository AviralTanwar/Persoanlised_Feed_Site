...
router.get('/', (req, res) => {
  const user = db.prepare(
    "SELECT user_id, firstname, lastname, username, title, location, timezone FROM tbl_user_info WHERE active = 1 AND deleted_at = '0000-00-00 00:00:00' LIMIT 1"
  ).get();
...
router.patch('/', (req, res) => {
  const { firstname, lastname, username, title, location, timezone } = req.body;
  db.prepare(`UPDATE tbl_user_info SET
    ...
    .run(firstname??null, lastname??null, username??null, title??null, location??null, timezone??null);
  const user = db.prepare(
    "SELECT user_id, firstname, lastname, username, title, location, timezone FROM tbl_user_info WHERE active=1 AND deleted_at='0000-00-00 00:00:00' LIMIT 1"
  ).get();
